...
prompt2:.asciiz "\nPodaj druga liczbe : "
iExc1:	.asciiz "\nPodawana liczba moze skaladac sie tlyko z cyfr kropki i znaku 'E' np 123.456E4"
iExc2:	.asciiz "\nPodana liczba nie moze miec wartosci zero"
iExc3:	.asciiz "\nPodana liczba musi byc liczba calkowita"
...
main:
	...
	beq $v0,2,inputExc2
	beq $v0,1,inputExc1
	move $s0,$v0			# $s0 - wskaznik na pamiec z pierwsza liczba
	move $s1,$v1
mainInput2:
	la $a0,prompt2
	li $v0,4
	...
	beq $v0,2,inputExc2
	beq $v0,1,inputExc1
	move $s2,$v0			# $s2 - wskaznik na pamiec z druga liczba
	move $s3,$v1
...
#--------f-cja licz¹ca NWD z dwoch podanych liczb--------#
# $a0 - wskaznik na pamiec z jedna liczba
# $a1 - ilosc cyfr w pierwszej liczbie
# $a2 - wskaznik na pamiec z druga cyfra
# $a3 - ilosc cyfr w drugiej liczbie
# $v0 - wskaznik na pamiec z wynikiem
# $v1 - ilosc cyfr wyniku
NWD:
	addi $sp,$sp,-32
	sw $s0,0($sp)
	sw $s1,4($sp)
...
NWDLoop:
	lb $t0,0($s2)
	beq $t0,48,NWDOut	# dopoki reszta nie rowna sie zero
		move $s4,$s2	# zapamietujemy dzielnik w pomocniczej zmiennej
		move $s5,$s3	
		move $a0,$s0
		move $a1,$s1
		move $a2,$s2
		move $a3,$s3
		jal division
		move $s2,$v1	# przeniesienie reszty do rejesrtu na dzielnik
		move $s3,$t8
		move $s0,$s4	# przeniesienie dzielnika do rejestru na dzielna
		move $s1,$s5
		j NWDLoop
...
# 1 - wpisano  cos innego niz cyfre, 2 - wartosc wpisanej cyfry wynosi 0, 3 - wpisana liczba nie jest calkowita
...
readString:
	 ...
	 li $s0,0		# flaga wskazujaca czy wpisana zostala kropka
	 li $s1,0		# licznik znakow po przecinku/kropce
	 li $s2,0		# flaga wskazujaca wpisanie znaku 'E'
	 li $s3,0		# wartosc po znaku 'E' (ascii 69)
	 li $a0,4
	 li $v0,9
...
readChar:li $v0,12
	 ...
	 move $t4,$v0
	 beq $v0,10,readOut	# jezeli wpisanym znakiem jest 'Enter' konczymy f-cje
	 subi $t4,$t4,48	# zamiana znaku na cyfre
	 
	 bne $t4,-2, readCnt5	# jezeli wpisanym znakiem byla '.'
	 bnez $s0,readExc1	# jezeli nie wpisano juz wczesniej kropki
	 li $s0,1		# ustawienie flagi kropki na true
	 j readCnt3
	 
readCnt5:	
	 bne $t4,21, readCnt4	# jezeli wpisanym znakiem bylo 'E'
	 bnez $s2,readExc1	# jezeli nie pisano juz znaku 'E' wczesniej
	 li $s2,1		# ustawienie flagi wpisania znaku 'E' na true
	 j readCnt3
readCnt4:	 
...
readCnt2:
	 ...
	 beqz $s0, readChar	# jezeli nie zostala jeszcze podniesiona flaga kropki nie rob nic
	 addi $s1,$s1,1		# jezeli jednak flaga jest podniesiona nalezy zarejestrowac ten znak aby potem sprawdzic czy liczba jest calkowita
	 j readChar		# pomin wczytywanie tego znaku
readCnt:
	bnez $s2,readE
	addi $t2,$t2,1		# dodaj jeden do ilosci wpisanych znakow 
	beqz $s0, readCnt6
	addi $s1,$s1,1		# jezeli zostala wpisana juz kropka nalezy zliczyc ten znak
readCnt6:	
...
readOut:
	beq $t2,0, readExc2
	sub $s3,$s3,$s1 	# od wykladnika odejmij ilosc znakow po przecinku
	bltz $s3,readExc3	# jezeli roznica jest mniejsza od zera liczba nie jest calkowita
readLoop:	
	blez $s3, readCnt7
	li $v0,48
	addi $t2,$t2,1		# dodajemu jeden do ilosci cyfr w liczbie
	sb $v0, 0($t3)		# zapisz znak w pamieci
	subi $t0,$t0,1		# odejmij jeden od ilosci pozostalej pamieci na znaki
	addi $t3,$t3,1		# przesun wskaznik o jeden znak w lewo
	subi $s3,$s3,1		# zmniejszamy licznik pozostalych zer do wpisania o jeden
	bgtz $t0,readLoop	# jezeli konczy sie nam zaalokowana pamiec
	li $a0,4
	li $v0, 9
	syscall			# nalezy zaalokowac nastepne cztery bajty
	addi $t0,$t0,4
